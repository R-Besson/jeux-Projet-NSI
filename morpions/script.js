...
{
    let startX = parseInt(start.getAttribute("x")),
        startY = parseInt(start.getAttribute("y"))

    let endX = parseInt(end.getAttribute("x")),
        endY = parseInt(end.getAttribute("y"))
    
    let squareSize = parseInt(start.getAttribute("width"))
...
function isLine(x, y, symbol)
{
    ...
    for (let dir = 0; dir < 4; dir++)
    {
        ...
        {
            let newX = x-step*dx, newY = y-step*dy
...
document.body.addEventListener('click', (event) => {
    ...
    if (!mouseIn(zone.x, zone.y, zone.width, zone.height, event.clientX, event.clientY)) return
